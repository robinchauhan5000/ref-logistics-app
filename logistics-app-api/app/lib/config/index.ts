...
// const apiUrl =
//   nconf.get('express').protocol +
//   (nconf.get('express').useFqdnForApis ? nconf.get('express').fqdn : nconf.get('express').ipAddress) +
//   ':' +
...
if (env !== 'development') {
  ...
  nconf.set('logisticsConfig:BPP_URI', process.env.BPP_URI)
  nconf.set('logistics:serverUrl', process.env.LOGISTICS_SERVER_URL)
